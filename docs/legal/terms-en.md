...
against that provider's own actual model list (and if the model isn't found, the list is shown to
pick from). It is now stated explicitly that **we have no way to know the cost** of a provider/model
the user chose — that is the user's responsibility alone.

Changes in version 1.7: a new section was added — **Your Own API Key (Bring Your Own Key)**,
section 12: an option to enter, in Settings, your own key with the Service's model provider, which
grants a second allowance the **same size** as your plan's quota once the regular one runs out. The
key is stored only in the user's browser, never with us. Sections after it were renumbered
(Changes to These Terms → 13, Intended Audience/Governing Law/Jurisdiction → 14, Contact → 15).

Changes in version 1.6: section 4 (Account and Registration) gained an **explicit enforcement
protocol** — a non-severe breach gets outreach and a chance to stop before suspension; a severe
breach (security, scraping, unlawful content, deliberate overload) can trigger immediate
suspension with the reason shown to the user; it is now stated that a breach-based suspension does
not entitle a refund, and the right to seek compensation for damage from misuse is reserved. This
is so the suspension right is a graduated, fair process rather than unbounded discretion — which
could itself read as an unfairly-prejudicial clause (see `docs/legal/LAWSUIT-EXPOSURE-2026-07-30.md`).

Changes in version 1.5: section 3 was split into two separate parts — the accuracy/"AS IS"
disclaimer versus a **liability limitation with a concrete cap** (the amount paid in the preceding
3 months, or ₪100 on the free plan), carved out for willful misconduct/gross negligence. This
followed a review finding that a blanket exemption from all liability is exposed to judicial
scrutiny under the Standard Contracts Law; a concrete cap survives that scrutiny better than a
blanket exemption (see `docs/legal/LAWSUIT-EXPOSURE-2026-07-30.md`, Finding B).
