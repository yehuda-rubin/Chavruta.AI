...
def _generate_lesson_from_hits(topic: str, hits, lang: str, he: bool, *, audience: str, grade_band: str,
    ...
    from chavruta.generation.grounded import (
        misattributed_quotes, misattribution_note, unverified_quotes)
    bad_q = unverified_quotes(fl + "\n" + ss, hits)
    # Misattribution runs on the LESSON TEXT ALONE, not on the source sheet. The sheet is assembled
    # mechanically by us from the citations, so its "**1. רש״י על סוכה 41**" heading sits directly
    # above that source's own verbatim text — feeding it to a checker that looks for a name near a
    # quote would report our own correct formatting as a fabrication, every time.
    wrong_name = misattributed_quotes(fl, hits)
    ...
    if wrong_name:
        caveats.append(misattribution_note("he" if he else "en", wrong_name))
